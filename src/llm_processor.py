...
class LLMProcessor:
    ...
    def _record_token_usage(self, drug_name: str, article_id: str, response_data: Dict):
        ...
        try:
            ...
            # 估算成本（根据智谱AI定价）
            # GLM-4: 输入 ¥0.1/千tokens，输出 ¥0.1/千tokens
    ...
    def get_token_statistics(self):
        ...
        try:
            if not os.path.exists(self.token_usage_file):
                return {"error": "统计文件不存在"}

            df = pd.read_csv(self.token_usage_file)
            ...
            return {
                ...
                'average_tokens_per_request': round(total_tokens / len(df), 2) if len(df) > 0 else 0
            }
        except Exception as e:
            return {"error": f"读取统计失败: {e}"}
...
现在请分析文献并输出JSON："""
    ...
    def _validate_and_fix_fields(self, result: Dict, article: Dict, article_id: str) -> Dict:
        ...
        for target in result['targets']:
            if isinstance(target, dict) and target.get('target_name'):
                ...
                target.setdefault('confidence_level', 'medium')  # 默认置信等级

                # 处理genes和pathways字段类型
    ...
    def batch_analyze_articles(self, drug_name: str, drug_index: int, articles: List[Dict]) -> List[Dict]:
        ...
        for i, article in enumerate(articles, 1):
            ...
            # 修改文章编号格式：D001.P01
            article_id = f"D{drug_index:03d}.P{i:02d}"  # D001.P01 格式
